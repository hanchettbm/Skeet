...
Bird* Game :: createBird()
{
   ...
   // TODO: Fill this in
   int myNumber = random(1, 4);
    
   if (myNumber == 1)
   {
       newBird = new Standard();
       int startingY = random(-200, 200);
       newBird->setPoint(Point(topLeft.getX(),startingY));
       int xVelNumber = random(3, 7);
       
       if (startingY >= 0)
       {
           int yVelNumber = random(-4, 0);
           newBird->setVelocity(Velocity(xVelNumber,yVelNumber));
       }
       
       else
       {
           int yVelNumber = random(1, 4);
           newBird->setVelocity(Velocity(xVelNumber,yVelNumber));
       }
  ...
  if (myNumber == 2)
  {
     ...
     int startingY = random(-200, 200);
     newBird->setPoint(Point(topLeft.getX(),startingY));
     int xVelNumber = random(2, 5);
      
     if (startingY >= 0)
     {
        int yVelNumber = random(-3, 0);
        newBird->setVelocity(Velocity(xVelNumber,yVelNumber));
     }
      
     else
     {
         int yVelNumber = random(1, 3);
         newBird->setVelocity(Velocity(xVelNumber,yVelNumber));
     }
